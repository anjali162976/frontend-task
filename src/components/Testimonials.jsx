
import logoWhite from "../assets/logo white.png";
function Testimonials() {
  return (
    <div>
      <div className="flex flex-col md:flex-row md:items-center gap-16 mb-20">
        <h2 className="bg-[#B9FF66] text-[#191A23] px-8 py-2 rounded-xl font-semibold text-4xl">
           Testimonials
        </h2>
        <p className="text-[#191A23] max-w-[500px] mt-6 md:items-center opacity-70 text-medium">
          Hear from Our Satisfied Clients: Read Our Testimonials to Learn More about Our Digital Marketing Services
        </p>
      </div>
      <section className="px-8 pt-24 pb-12 bg-[#191A23] text-white rounded-[40px]">

{/* Testimonial Wrapper */}
<div className="flex gap-8 overflow-hidden">

        {/* Card 1 */}
        <div className="border border-[#B9FF66] rounded-3xl p-8 max-w-xl">
          <p className="text-lg leading-relaxed mb-6">
            “We have been working with Positivus for the past year and have seen
            a significant increase in website traffic and leads. Their team is
            professional, responsive, and truly cares about our success.”
          </p>

          <div>
            <p className="font-semibold text-[#B9FF66]">
              John Smith
            </p>
            <p className="text-sm text-gray-400">
              Marketing Director at XYZ Corp
            </p>
          </div>
        </div>

        {/* Card 2 */}
        <div className="hidden md:block border border-gray-600 rounded-3xl p-8 max-w-xl opacity-60">
          <p className="text-lg leading-relaxed mb-6">
           “We have been working with Positivus for the past year and have seen
            a significant increase in website traffic and leads. Their team is
            professional, responsive, and truly cares about our success.”
          </p>

          <div>
            <p className="font-semibold text-[#B9FF66]">
               John Smith
            </p>
            <p className="text-sm text-gray-400">
             Marketing Director at XYZ Corp
            </p>
          </div>
        </div>

        {/* Card 3*/}
        <div className="hidden md:block border border-gray-600 rounded-3xl p-8 max-w-xl opacity-60">
          <p className="text-lg leading-relaxed mb-6">
           “We have been working with Positivus for the past year and have seen
            a significant increase in website traffic and leads. Their team is
            professional, responsive, and truly cares about our success.”
          </p>

          <div>
            <p className="font-semibold text-[#B9FF66]">
               John Smith
            </p>
            <p className="text-sm text-gray-400">
             Marketing Director at XYZ Corp
            </p>
          </div>
        </div>

    </div>

   
<div className="mt-10 flex items-center justify-between px-2 md:px-10">
 
  <button
    type="button"
    className="text-white/40 hover:text-white transition text-4xl leading-none"
    aria-label="Previous testimonial"
  >
    &#8592;
  </button>

  {/* icons  */}
  <div className="flex items-center gap-4">
    <img src={logoWhite} alt="" className="h-4 w-4 opacity-100 rotate-45" />
    <img src={logoWhite} alt="" className="h-4 w-4 opacity-40 rotate-45" />
    <img src={logoWhite} alt="" className="h-4 w-4 opacity-40 rotate-45" />
    <img src={logoWhite} alt="" className="h-4 w-4 opacity-40 rotate-45" />
    <img src={logoWhite} alt="" className="h-4 w-4 opacity-40 rotate-45" />
  </div>

  {/* Next arrow */}
  <button
    type="button"
    className="text-white/40 hover:text-white transition text-4xl leading-none"
    aria-label="Next testimonial"
  >
    &#8594;
  </button>
</div>

    </section>
    </div>
  )
}

export default Testimonials



